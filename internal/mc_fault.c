...
#define MC_UNDER_VOLTAGE_LIMIT         ((MC_UNDER_VOLTAGE_EVENT_LEVEL   > (MC_STARTUP_VOLTAGE)) ? MC_UNDER_VOLTAGE_EVENT_LEVEL   : (MC_STARTUP_VOLTAGE))
#define MC_UNDER_VOLTAGE_LIMIT_RESTORE ((MC_UNDER_VOLTAGE_RESTORE_LEVEL > (MC_STARTUP_VOLTAGE)) ? MC_UNDER_VOLTAGE_RESTORE_LEVEL : (MC_STARTUP_VOLTAGE + 1.0))
...
void MC_Fault_LimitsCheck(mc_analog_id_t id, uint16_t res)
{
    ...
    static uint16_t ovoltage = MC_VOLTAGE_TO_ADC(MC_OVER_VOLTAGE_EVENT_LEVEL);
    static uint16_t uvoltage = MC_VOLTAGE_TO_ADC(MC_UNDER_VOLTAGE_LIMIT);
    static uint16_t otemp = MC_CELSIUS_TO_ADC(MC_OVER_TEMPERATURE_EVENT_LEVEL);
    ...
    switch(id)
    {
        case AID_VOLTAGE:   if(res > ovoltage) 
                            {
                                ...
                                {
                                    ov_status = true;
                                    ovoltage = MC_VOLTAGE_TO_ADC(MC_OVER_VOLTAGE_RESTORE_LEVEL);
                            ...
                            else
                            {
                                if(res < uvoltage)
                                {
                                    ...
                                    {
                                        uv_status = true;
                                        uvoltage = MC_VOLTAGE_TO_ADC(MC_UNDER_VOLTAGE_LIMIT_RESTORE);
                                ...
                                else
                                {
                                    ...
                                    {
                                        ov_status = false;
                                        ovoltage = MC_VOLTAGE_TO_ADC(MC_OVER_VOLTAGE_EVENT_LEVEL);
                                        ...
                                        {
                                            uv_status = false;
                                            uvoltage = MC_VOLTAGE_TO_ADC(MC_UNDER_VOLTAGE_LIMIT);
        ...
        case AID_TEMPERATURE:
                            if(res >= otemp)
                            {
                                ...
                                {
                                    temp_status = true;
                                    otemp = MC_CELSIUS_TO_ADC(MC_OVER_TEMPERATURE_RESTORE_LEVEL);
                            ...
                            else
                            {
                                ...
                                {
                                    temp_status = false;
                                    otemp = MC_CELSIUS_TO_ADC(MC_OVER_TEMPERATURE_EVENT_LEVEL);
        ...
        case AID_CURRENT:   ires = (int16_t)res;
                            if(ires < 0)
                                ires = 0 - ires;
                            if(ires >= MC_CURENT_TO_ADC(MC_OVER_CURRENT_AVG_EVENT_LEVEL))
